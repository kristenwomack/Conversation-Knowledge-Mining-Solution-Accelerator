...
import pandas as pd
import json
from models.input_models import ChartFilters
...
def adjust_dates(conn: Connection):
    ...
    try:
        ...
        logging.error(f"Error adjusting dates: {e}", exc_info=True)
        raise


def get_filters_metrics(conn: Connection) -> list:
    ...
    try:
        ...
        logging.error(f"Error getting filters metrics: {e}", exc_info=True)
        raise


def get_charts_metrics(conn: Connection, chart_filters: ChartFilters = '') -> list:
    ...
    try:
        ...
        logging.error(f"Error getting charts metrics: {e}", exc_info=True)
        raise
